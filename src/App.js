...
useEffect(()=> {
  localStorage.setItem('tache', JSON.stringify(item))
    ...
    setIsEditItem(null)
          ...
          if (userInput || desc) {
            const newInput = { id: new Date().getTime().toString(),userInput, desc };
  ...
  return (
    ...
    <TodoList handleSubmit ={handleSubmit} desc={desc} userInput={userInput} setDesc={setDesc} setUserInput={setUserInput} toggleSubmit={toggleSubmit} />
    <Notes item = {item} removeTodo ={removeTodo} modifTodo={modifTodo} />
    <button className="btn-result" onClick={() => del([]) }>Tout delete</button>
    </>
    )
}
